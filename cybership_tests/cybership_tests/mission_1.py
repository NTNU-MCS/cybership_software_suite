...
class ActionRunner(Node):
    def __init__(self):
        ...
        self.create_timer(0.05, self._experiment_publisher)
        ...
        self.actions = [
            ...
            #     "parameters": {
            #         "force_x": lambda: self.rng.uniform(0.7, 1.3),
            #         "force_y": lambda: self.rng.uniform(-0.0, 0.0),
            #         "torque_z": lambda: self.rng.uniform(-0.0, 0.0),
            #         "duration": 20.0,
    ...
    def call_init_services(self) -> None:
        ...
        self.get_logger().info(f"Waiting for pose action server...")
        self.pose_action_client.wait_for_server()

        self.get_logger().info(f"Waiting for pose enable server ...")
        self.pose_enable_client.wait_for_service()

        self.get_logger().info(f"Waiting for vel enables server ...")
        self.vel_enable_client.wait_for_service()

        self.get_logger().info(f"Waiting for Force mux server...")
        self.mux_add_client.wait_for_service()

        self.get_logger().info(f"Waiting for Force mux server...")
        self.mux_select_client.wait_for_service()
    ...
    def abort_condition(self) -> bool:
        ...
        return (
            not(
                (-5.0 < self.current_odom.pose.pose.position.x < 4.0)
            and (-3.0 < self.current_odom.pose.pose.position.y < 2.0)
            )
        )
    ...
    def set_enable(self, client: rclpy.client.Client, enable: bool) -> None:
        ...
        """
        self.get_logger().info(f"Setting {client} enable={enable}")
        req = SetBool.Request()
        req.data = enable
        future = client.call_async(req)
        rclpy.spin_until_future_complete(self, future)
        if future.result().success:
            self.get_logger().info(f'Service {client} set enable={enable}')
        else:
            self.get_logger().error(f'Failed to set {client} enable={enable}')
    ...
    def run_pose_action(self, **params: Any) -> None:
        ...
        """
        resolved_params = {k: v() if callable(
            v) else v for k, v in params.items()}
        ...
        while rclpy.ok():
            ...
            if elapsed >= grace and self.abort_condition():
                ...
                self.set_enable(self.pose_enable_client, False)
                raise RuntimeError(
                    'Abort condition met during pose action, aborting')
    ...
    def run_force_action(self, **params: Any) -> None:
        ...
        while rclpy.ok() and elapsed < duration:
            ...
            # if np.mod(elapsed, period) > 0.01:
            #     # Skip this iteration if not at the right frequency
            #     continue

            if abs(last_update_elapsed - elapsed) < period:
        ...
        for _ in range(self.repeat):
            ...
            for action in self.actions:
                ...
                try:
                    ...
                    abort_action = action.get(
                        'abort_action', self.default_abort_action)
                    ...
                    try:
                        ...
                        self.get_logger().error(
                            f'Abort action failed too!: {abort_e}')
                        self.set_enable(self.pose_enable_client, False)
                        self.set_enable(self.vel_enable_client, False)
    ...
    def odom_callback(self, msg):
        ...
        # if self.experiment_enabled:
        #     # Publish odometry for experiment if enabled
        #     self.experiment_odom_pub.publish(msg)
        #     self.experiment_flag_pub.publish(Bool(data=True))
        #     self.experiment_force_pub.publish(self._last_force)
