...
class SquareMoveController(Node):
    def __init__(self):
        ...
        #---------------------------------------
        # Proportional gains
        self.kp_xy = 1.3     # cross-track & along-track
        self.kp_yaw = 0.6    # heading
        self.kp_speed = 0.6  # forward speed

        # Integral gains
        self.ki_xy = 0.2     # cross-track integrator
        self.ki_yaw = 0.05    # heading integrator (set to 0 if not desired)
        self.ki_speed = 0.01  # forward speed integrator (set to 0 if not desired)
        ...
        self.get_logger().info("Square Move Controller with cross-track integral initialized.")
    ...
    def control_loop(self):
        ...
        tau_ct_x = -(
            self.kp_xy * cross_x
            + self.ki_xy * self.cross_x_int
        )
        tau_ct_y = -(
            self.kp_xy * cross_y
            + self.ki_xy * self.cross_y_int
        )
        ...
        speed_control = (
            ...
            + self.ki_speed * self.speed_err_int
        )
        ...
        wrench_msg.force.y = float(tau_y) * 2.0
