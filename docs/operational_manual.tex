...
\begin{document}
	...
	For the first time user it might seem that everything is in order. One should be able to see the rigid body defined by the markers as well as its coordinates in 6DOF, but QTM uses the centroid of the markers defining a rigid body as the body frame origin which does not correspond to the vessel geometric center. To remedy this all Cybership vessels should have a reference marker marked by a black line on the marker stem with its position relative to the vessel geometric center and the vessel waterline written close to it. The current values at the time of writing are noted below in tables for Voyager (Table~\ref{tab:voyager_ref_marker}) and Drillship (Table~\ref{tab:drillship_ref_marker}).
	...
	\end{center}
	Enter the values for the vessel in question and press \emph{OK}, press apply and verify that the body has moved the origin of the system. 
